...
启动脚本还会应用 AI Tutor demo 的 `demo_ai_tutor_*` 五张表（`sql/082_demo_ai_tutor.sql`），并在表已存在但缺列时补 `demo_ai_tutor_conversation.verla_conversation_id`（`sql/083_demo_ai_tutor_verla_link.sql`，AI Tutor 并入 Verla 主线事件通道后的 SSE 会话键）。082 此前不在自动补齐清单里，需要手动导入，漏跑会让 AI Tutor 接口一调就报表不存在。
