...
这些 mock 数据用于本地页面和套餐内容联调；`start-mock.sh` 默认设置 `BILLING_CHECKOUT_MOCK_ENABLED=true` 和 `PAYMENT_CHECKOUT_MOCK_ENABLED=true`，会员购买会本地更新 `user_subscriptions`、写入 completed `recharge_orders`，并直接回跳 `/payment-success?session_id=mock_cs_...`。如需真实 Stripe checkout 跳转，需设置 `BILLING_CHECKOUT_MOCK_ENABLED=false PAYMENT_CHECKOUT_MOCK_ENABLED=false`，并按 `sql/v2_billing_stripe_sandbox_ids.example.sql` 写入对应 sandbox `price_...`。
