...
public class CapabilityRunDispatchQueueNotifier implements CapabilityRunDispatchQueueEvents {
    ...
    public void notifyDeferred(MqOutbox message) {
        ...
        VerlaAgentEventType eventType = resolveQueuedEventType(message.getAction());
        if (eventType == null) {
            return;
        }
        ...
        int queuePosition = mqOutboxRepository.countDeferredCapabilityRunAhead(
                message.getId(), message.getAction(), createdAt);
    ...
    private static VerlaAgentEventType resolveQueuedEventType(String action) {
        if (VerlaCommandAction.CMD_DETECTION_RUN.getCode().equals(action)) {
            return VerlaAgentEventType.AI_DETECTION_RUN_DISPATCH_QUEUED;
        }
    ...
    private void scheduleAfterCommitPublish(Long conversationId, VerlaSseEventPayload payload) {
        ...
        Runnable publish = () -> {
            ...
            if (publisher == null) {
                log.debug("[Verla/capability-run-dispatch] no SSE publisher, skip live push cid={}",
                        conversationId);
                return;
            }
    ...
    private String serializeEnvelope(VerlaEventEnvelope envelope) {
        try {
            ...
            log.warn("[Verla/capability-run-dispatch] envelope serialize failed: {}", e.getMessage());
